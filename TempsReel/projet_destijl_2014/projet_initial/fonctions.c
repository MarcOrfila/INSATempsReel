...
void connecter(void * arg) {
    ...
    while (1) {
        ...
        // Si la connection n'est pas encore faite, on la fait
        if (status != STATUS_OK){
...
void communiquer(void *arg) {
	...
	while(1){
    	...
    	// Variable représentant le nombre de tentatives de connection  
    	rt_mutex_acquire(&mutexErreur, TM_INFINITE);
    	tentatives = 0;
    	rt_mutex_release(&mutexErreur);
    	
    	// Variable représentant le mouvement du robot
    	rt_mutex_acquire(&mutexMove, TM_INFINITE);
    	move->set(move,0,0);
    	rt_mutex_release(&mutexMove);
    	
    	// Variable représentant l'état de la caméra 
    	rt_mutex_acquire(&mutexCam, TM_INFINITE);
    	etatCamera = ACTION_STOP_COMPUTE_POSITION;
    	rt_mutex_release(&mutexCam);
    	
    	// Variable représentant la position du robot
    	rt_mutex_acquire(&mutexPosition, TM_INFINITE);
    	position = NULL;
    	rt_mutex_release(&mutexPosition);
    	
    	// Variable représentant l'arène
    	rt_mutex_acquire(&mutexArene, TM_INFINITE);
    	arene = NULL;
    	rt_mutex_release(&mutexArene);
    	
    	// Vartiable représentant l'état de la mission 
    	rt_mutex_acquire(&mutexEtatMission, TM_INFINITE);
    	etatMission = TERMINATED;
    	rt_mutex_release(&mutexEtatMission);
    	
    	// Si le status est nul, l'ouverture du serveur a fonctionné
    	if (status == 0){
    		// Instruction de controle 
