		...
		while (etatMissionLocal != NONE) {
			...
			switch(etatMissionLocal) {
				...
				case PENDING : //mission en cours à effectuer
					...
					/*	if ((fabs(angle) > DELTA_A) && (busy == 0)) {
						//if ((angle > DELTA_A || angle <(-DELTA_A)) && (busy = 0)){
							//le robot a besoin de tourner
							if (angle > 180)
								angle = angle - 360;
							if (angle < - 180)
								angle = angle + 360;
							...
							while (busy == 1) {
								...
								again = 0;
								while(!again) {
									printf("-------------- AVANT BUSY : BESOIN DE TOURNER ------------ \n");
									rt_mutex_acquire(&mutexRobot, TM_INFINITE);
									status = d_robot_is_busy(robot, &busy);
									rt_mutex_release(&mutexRobot);
									again = verifierPerteConnexion(status);
									printf("---------------- AGAIN BUSY : BESOIN DE TOURNER : %d--------\n",again);									
								}
								printf("----------------- BOUCLE BUSY ------------- \n");
							}
							//le robot a fini sa tâche
						...
						while (busy == 1) {
							...
							again = 0;
							while (!again) {
								printf("----------------- AVANT BUSY : TOUT DROIT -----------\n");
								rt_mutex_acquire(&mutexRobot, TM_INFINITE);
								status = d_robot_is_busy(robot, &busy);
								rt_mutex_release(&mutexRobot);
								again = verifierPerteConnexion(status);
								printf("---------------- AGAIN BUSY : TOUT DROIT : %d--------\n",again);
							}	
						}
						...
						again = 0;
						while (!again) {
							printf("--------------- AVANT TOUT DROIT ------------\n");
							rt_mutex_acquire(&mutexRobot, TM_INFINITE);
							status = d_robot_move(robot, distance);
							rt_mutex_release(&mutexRobot);
							again = verifierPerteConnexion(status);
							printf("---------------- AGAIN TOUT DROIT: %d--------\n",again);
						}
					}*/
					break;
